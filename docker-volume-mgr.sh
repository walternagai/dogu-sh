...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --remove|-r)
            ...
            ACTION="remove"; VOLUME_NAME="$2"; shift 2 ;;
        --remove-orphans) ACTION="remove-orphans"; shift ;;
        --backup|-b)
            [[ -z "${2-}" ]] && { echo "Flag --backup requer um valor" >&2; exit 1; }
            ACTION="backup"; VOLUME_NAME="$2"; shift 2 ;;
        ...
        --help|-h)
            ...
            exit 0
            ;;
        --version|-V) echo "docker-volume-mgr.sh $VERSION"; exit 0 ;;
        --) shift; break ;;
