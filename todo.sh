...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -d|--done)
            [[ -z "${2-}" ]] && { echo "Flag --done requer um valor" >&2; exit 1; }
            ACTION="done"; ARG1="$2"; shift 2 ;;
        -u|--undo)
            [[ -z "${2-}" ]] && { echo "Flag --undo requer um valor" >&2; exit 1; }
            ACTION="undo"; ARG1="$2"; shift 2 ;;
        -r|--remove)
            [[ -z "${2-}" ]] && { echo "Flag --remove requer um valor" >&2; exit 1; }
            ACTION="remove"; ARG1="$2"; shift 2 ;;
        -l|--list)
            [[ -z "${2-}" ]] && { echo "Flag --list requer um valor" >&2; exit 1; }
            ACTION="list"; ARG1="${2:-pending}"; shift 2 ;;
