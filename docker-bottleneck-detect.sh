...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --threshold|-t)
            [[ -z "${2-}" ]] && { echo "Flag --threshold requer um valor" >&2; exit 1; }
            ALERT_THRESHOLD="$2"; shift 2 ;;
        --waste|-w)
            [[ -z "${2-}" ]] && { echo "Flag --waste requer um valor" >&2; exit 1; }
            WASTE_THRESHOLD="$2"; shift 2 ;;
        --watch|-W)
        ...
        --help|-h)
            ...
            exit 0
            ;;
        --version|-V) echo "docker-bottleneck-detect.sh $VERSION"; exit 0 ;;
        --) shift; break ;;
